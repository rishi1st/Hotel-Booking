     ...
     <form action="">
       ...
       <div className='grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap'>
        {
          Object.keys(images).map((key)=>(
            <label htmlFor= {`roomImages${key}`} key={key}>
              <img src= {images[key] ? URL.createObjectURL(images[key]) :assets.uploadArea} alt={`roomImages${key}`}  className='max-h-13 cursor-pointer opacity-80'/>
       ...
       <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
        <div className='flex-1 max-w-48'>
          ...
          <select  
           onChange={e => setInput({...inputs , roomType:e.target.value})}
          name="" id="" className='border opacity-70 border-gray-300 mt-1 rounded p-2 w-full'>
        ...
        <div>
          ...
          <input type="number" placeholder='0' className='border border-gray-300 mt-1  rounded p-2 w-24'
           ...
           onChange={e => setInput({...inputs , pricePerNight:e.target.value})}
          />
        </div>
       </div>
       ...
       <div className='flex flex-col flex-wrap  mt-1 text-gray-400 max-w-sm'>
           {
            Object.keys(inputs.amenities).map((amenity , index)=>(
              <div key={index} className='flex gap-2'>
                   ...
                   onChange={()=> setInput({...inputs , amenities:{...inputs.amenities , [amenity]:!inputs.amenities[amenity]}})}
                   />
